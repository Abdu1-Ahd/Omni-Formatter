import os, sys, math; import datetime
import json,re
from collections import defaultdict,Counter; from itertools import chain

class ProductData:
  def __init__(self,id,name, price,category): self.id= id;self.name=name;self.price = price; self.category= category
  def get_discounted_price( self , discount_rate ):
     return self.price-( self.price* discount_rate)
  def __str__(self):return "Product: "+str(self.id)+" - "+self.name
...
class inventory_manager:
    ...
    def add_item(self,product,qty):
        if product.id in self.items:self.items[product.id]['qty']+=qty
        else:
...
class order_processor:
  def __init__(self, tax_rate=0.05): self.tax_rate=tax_rate;self.orders=[]
...
def calculate_complex_statistics( inventory ):
      ...
      for pid,data in inventory.items.items():
            ...
            if prc<50:
                  price_ranges['low']+=1
            elif prc<200:price_ranges['medium']+=1
            else:
                  price_ranges['high']+=1
...
class UserAccount:
    def __init__( self, username , email, role="customer"):
      self.username=username; self.email= email; self.role=role
      self.is_active= True
...
def run_simulation():
      ...
      print("Category distribution:");print(cats)
      print("Price ranges:");print(ranges)
...
def test_linked_list():
    ...
    for i in range(1, 11): dll.append(i)
    dll.prepend(0)
    dll.delete(5)
...
def test_event_dispatcher():
    ...
    def on_user_login(data): print(f"User logged in: {data['username']}")
    def on_user_logout(data): print(f"User logged out: {data['username']}")
    def log_event(data): print(f"Event logged: {data}")
