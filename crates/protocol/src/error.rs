...
pub enum FormatError {
    ...
    ModuleError { language_id: String, message: String },
    ...
    ModuleVerificationFailed { language_id: String, expected: String, actual: String },
...
impl std::fmt::Display for FormatError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FormatError::FileTooLarge { size_bytes, limit_bytes } => {
                write!(f, "File too large: {} bytes (limit: {} bytes)", size_bytes, limit_bytes)
            }
            FormatError::ParseFailed { message } => {
                write!(f, "Parse failed: {}", message)
            }
            FormatError::SyntaxErrorInRange { offset } => {
                write!(f, "Syntax error at byte offset {} — range formatting skipped", offset)
            }
            FormatError::ModuleError { language_id, message } => {
            ...
            FormatError::IdempotencyViolation { language_id, .. } => {
                write!(f, "IDEMPOTENCY VIOLATION in {}: format(format(x)) != format(x)", language_id)
            }
