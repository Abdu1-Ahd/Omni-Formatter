//! Dart / Kotlin / Swift Mobile Platform Structural Formatter
//!
//! Upgrades the previous naive brace counter (which counted `{` inside
//! string literals, causing indentation corruption on interpolated strings).
//!
//! Dart in particular is heavily affected because `"value is ${x + 1}"` would
//! register as an open-brace and shift the rest of the file.
//!
...
fn count_brace_delta(line: &str) -> (usize, usize) {
    ...
    while let Some(c) = chars.next() {
        match c {
            ...
            // Dart string interpolation: ${...} — skip the opening $ before {
            '$' if !in_str => {}
            '{' if !in_str => opens += 1,
            '}' if !in_str => closes += 1,
            _ => {}
...
mod tests {
    ...
    fn format_idempotent() {
        ...
        assert_eq!(first, second, "lang-mobile must be idempotent");
    }
    ...
    fn brace_in_string_not_counted() {
        // Dart: "value is ${x + 1}" — the { inside ${} must not count as a scope opener
        let src = b"var msg = \"value is ${x + 1}\";\nvar y = 2;\n";
        let result = format(src, &cfg()).unwrap();
        let s = std::str::from_utf8(&result).unwrap();
        let y_line = s
            .lines()
            .find(|l| l.contains("var y"))
            .expect("var y missing");
        assert_eq!(
            y_line.len() - y_line.trim_start().len(),
            0,
            "interpolation brace must not increase indent:\n{}",
            s
        );
    }
